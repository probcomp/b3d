...
def create_keypoints_gif(data: FeatureTrackData, output_path='output.gif', res: float=200, fps: float=10):
    ...
    for t in range(0, Nframe, frame_step):  # Skip every "frame_step"
        ...
        # keypoints_image = rgb_image.copy() # to overlay keypoints on rgb
        ...
        for kp in visible_keypoints:
            cv2.circle(keypoints_image, (int(kp[0]), int(kp[1])), 5, (0, 255, 0), -1)
        ...
        cv2.putText(labeled_rgb, 'RGB', (10, 50), font, font_scale, color, thickness)
        cv2.putText(labeled_depth, 'Depth', (10, 50), font, font_scale, color, thickness)
        cv2.putText(labeled_keypoints, 'Visible Keypoints', (10, 50), font, font_scale, color, thickness)
