...
def get_data_path(datafolder: str, scenefolder: str = None) -> Path:
...
def get_assets_path(datafolder: str, data_class: str, scenefolder: str = None) -> Path:
...
def get_unity_data_folder_path(
    ...
    raise FileNotFoundError(
        f"Data name '{data_name}' not found in any scenefolder under data class '{data_class_dir}'."
    )
...
def get_unity_data_path(
    ...
    if file_path.exists():
        ...
        print(
            f"'{data_name}' only exists as {data_class_dir_map(data_class)} with the following settings:"
        )
