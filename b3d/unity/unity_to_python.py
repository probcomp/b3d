...
QUAT_90_DEG_X_Pose = Pose(jnp.array([0., 0., 0.]), QUAT_90_DEG_X)
...
def clamp_and_replace_with_nan(
    ...
    array_clamped = array_clamped.at[..., 0].set(jnp.where(mask_x, jnp.nan, array_clamped[..., 0]))
    array_clamped = array_clamped.at[..., 1].set(jnp.where(mask_y, jnp.nan, array_clamped[..., 1]))
