...
def get_pixel_color(
    ...
    particle_widths,
    particle_colors,
    triangle_index_to_particle_index,
    vertices,
    faces,
    vertex_colors,
):
    ...
    particle_indices_in_window = triangle_index_to_particle_index[
        triangle_intersected_padded_in_window - 1
    ]
    ...
    particle_centers_on_image_plane = (
        particle_centers_in_window[..., :2] / particle_centers_in_window[..., 2:]
    )
    particle_pixel_coordinate = particle_centers_on_image_plane * jnp.array(
        [fx, fy]
    ) + jnp.array([cx, cy])

    distance_on_image_plane = jnp.linalg.norm(
        particle_pixel_coordinate - jnp.array([ij[1], ij[0]]) + 1e-5, axis=-1
    )

    particle_width_on_image_plane = (
        particle_widths_in_window / (particle_centers_in_window[..., 2] + 1e-10) + 1e-10
    )
    # return jnp.array([jnp.sum(particle_width_on_image_plane, axis=[0,1])]) # checks out

    scaled_distance_on_image_plane = (
        distance_on_image_plane / particle_width_on_image_plane / 20.0
    )
    scaled_distance_on_image_plane_masked = scaled_distance_on_image_plane * (
        triangle_intersected_padded_in_window > 0
    )
    ...
    color_image = jnp.sum(
        particle_colors_in_window_masked * influence[..., None], axis=[0, 1]
    ) / (influence.sum() + 1e-10)
    depth_image = jnp.sum(particle_depth_in_window * influence, axis=[0, 1]) / (
        influence.sum() + 1e-10
    )
...
def render(particle_centers, particle_widths, particle_colors):
    ...
    jj, ii = jnp.meshgrid(jnp.arange(image_size[1]), jnp.arange(image_size[0]))
    ijs = jnp.stack([ii, jj], axis=-1)
    rgb_image, depth_image = get_pixel_color(
        ijs,
        triangle_intersected_padded,
        particle_centers,
        particle_widths,
...
particle_widths = jax.random.uniform(
    jax.random.PRNGKey(0), (N,), minval=0.001, maxval=0.1
)
particle_colors = jax.random.uniform(
    jax.random.PRNGKey(0), (N, 3), minval=-1.0, maxval=1.0
)


ij = jnp.array([157, 85])
# 3 unit tests on 3 arguments
rgb_image, depth_image, triangle_id_image = render_jit(
...
for t in range(100):
    ...
    loss, (grad_centers, grad_widths, grad_colors) = loss_func_grad(
        particle_centers, particle_widths, particle_colors, gt_image, gt_depth
    )
    particle_centers -= 1 * grad_centers
    # particle_widths -= 1 * grad_widths
    particle_colors -= 1000 * grad_colors
    rgb_image, depth_image = render_jit(
        particle_centers, particle_widths, particle_colors
    )[:2]
    rr.log("image/reconstruction", rr.Image(rgb_image[..., :3]))
    rr.log("depth/reconstruction", rr.DepthImage(depth_image))
