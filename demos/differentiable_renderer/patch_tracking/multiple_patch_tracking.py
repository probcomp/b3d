...
rots = du.vec_transform_axis_angle(
...
xyzs_C = b3d.xyz_from_depth_vectorized(
...
(patch_vertices_P, patch_faces, patch_vertex_colors, Xs_WP) = jax.vmap(
    get_patches, in_axes=(0,)
)(centers)
...
model = m.multiple_object_model_factory(renderer, likelihood, hyperparams)
...
def importance_from_pos_quat_v3(positions, quaternions, timestep):
    poses = jax.vmap(
        lambda pos, quat: Pose.from_vec(jnp.concatenate([pos, quat])), in_axes=(0, 0)
...
for i in range(observed_rgbds.shape[0]):
    ...
    rr.log(
        ...
        rr.Points3D(
            ...
            colors=np.repeat(
                np.array([0, 0, 255])[None, ...], positions[i].shape[0], axis=0
            ),
        ),
    )
