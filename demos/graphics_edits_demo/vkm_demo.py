...
point_cloud = camera_poses_over_time[0].apply(
    jax.image.resize(
...
for t in range(start_t):
    ...
    rgb_rerender = renderer.render(
        camera_poses_over_time[t].inv().apply(background_vertices),
        background_faces,
...
masks_concat = jnp.stack(
    [b3d.carvekit_get_foreground_mask(r) for r in rgbs_for_acquisition]
)

mask_0 = b3d.carvekit_get_foreground_mask(rgbs_for_acquisition[0])

point_cloud_at_start = camera_poses_over_time[start_t].apply(xyz_for_acquisition[0])[
    mask_0
]
rr.log(
    "cloud",
    rr.Points3D(
        point_cloud_at_start, colors=rgbs_for_acquisition[0][mask_0].reshape(-1, 3)
    ),
)
...
def merge_objects(objects, poses=None):
    if poses:
        merged_vertices = jnp.concatenate(
            [poses[i].apply(v) for (i, (v, _, _)) in enumerate(objects)], axis=0
        )
    else:
    ...
    merged_faces = jnp.concatenate(
        [f + vertices_cumsum[i] for i, (_, f, _) in enumerate(objects)], axis=0
    )
    merged_vertex_colors = jnp.concatenate([c for _, _, c in objects], axis=0)

    vertex_to_index = jnp.concatenate(
        [jnp.full(v.shape[0], i) for i, (v, _, _) in enumerate(objects)]
    )
...
merged_vertices, merged_faces, merged_vertex_colors, vertex_to_index = merge_objects(
    objects, poses=poses
)
...
for t in range(start_t, end_t):
    if t % 5 == 0:
        occ_free_occl_, colors_per_voxel_ = (
            voxel_occupied_occluded_free_parallel_camera_depth(
                camera_poses_for_acquisition[: t - start_t],
                rgbs_for_acquisition[: t - start_t],
                xyz_for_acquisition[..., 2][: t - start_t] * masks_concat[: t - start_t]
                + (1.0 - masks_concat[: t - start_t]) * 5.0,
                grid,
                fx,
                fy,
                cx,
        ...
        merged_vertices, merged_faces, merged_vertex_colors, vertex_to_index = (
            merge_objects(objects, poses)
        )

    rr.set_time_sequence("frame", t)
    rgb_rerender = renderer.render(
        camera_poses_over_time[t].inv().apply(merged_vertices),
        merged_faces,
...
waypoints = [
    ...
    jnp.array([-0.2, -0.1, 0.4, 0.5]),
    jnp.array([0.3, -0.1, 0.4, 0.5]),
    jnp.array([0.0, 0.1, 0.2, 1.0]),
]
...
for i in range(len(interpolated_waypoints)):
    new_vertex_colors = adjust_vertex_colors(
        transformed_vertices,
        merged_faces,
        merged_vertex_colors,
        interpolated_waypoints[i, :3],
        ambient_light=float(interpolated_waypoints[i, 3]),
    )
    rr.set_time_sequence("frame", t + i)
    rgb_rerender = renderer.render(
        transformed_vertices, merged_faces, new_vertex_colors
    )
    rr.log("rgb/rerender", rr.Image(jnp.clip(rgb_rerender, 0.0, 1.0)))
    rr.log(
        "rgb",
...
for t in range(15):
    ...
    rgb_rerender = renderer.render(
        camera_poses_over_time[-1 - ticker].inv().apply(merged_vertices),
        merged_faces,
        merged_vertex_colors,
    )
    rr.log("rgb/rerender", rr.Image(jnp.clip(rgb_rerender, 0.0, 1.0)))
    rr.log(
        ...
        rr.Image(
            jax.image.resize(
                video_input.rgb[-1 - ticker] / 255.0,
...
objects[3] = mesh_list
objects[3][0] = objects[3][0][:, jnp.array([0, 2, 1])] * jnp.array(
    [1.0, -1.0, 1.0]
...
mesh_path = os.path.join(
    b3d.get_root_path(),
    "assets/shared_data_bucket/ycb_video_models/models/021_bleach_cleanser/textured.obj",
)
mesh = trimesh.load(mesh_path)
mesh_list = list(b3d.get_vertices_faces_colors_from_mesh(mesh))
objects[2] = mesh_list
...
mesh_path = os.path.join(
    b3d.get_root_path(),
    "assets/shared_data_bucket/ycb_video_models/models/025_mug/textured.obj",
)
mesh = trimesh.load(mesh_path)
mesh_list = list(b3d.get_vertices_faces_colors_from_mesh(mesh))
objects[1] = mesh_list
objects[1][0] = objects[1][0][:, jnp.array([0, 2, 1])] * jnp.array(
    [1.0, -1.0, 1.0]
) + jnp.array([0.0, 0.07, 0.0])
...
(
    merged_vertices_no_background,
    merged_faces_no_background,
    merged_vertex_colors_no_background,
    vertex_to_index_no_background,
) = merge_objects(old_objects[1:], poses[1:])

old_objects2 = [[jnp.copy(a) for a in b] for b in old_objects]
for i, color in [
    (1, jnp.array([0.6, 0.05, 0.05])),
    (2, jnp.array([0.05, 0.6, 0.05])),
    (3, jnp.array([0.05, 0.05, 0.6])),
]:
    object_pose = poses[i]
...
merged_vertices2, merged_faces2, merged_vertex_colors2, vertex_to_index2 = (
    merge_objects(old_objects2[1:], poses[1:])
)

points = merged_vertices[vertex_to_index > 0]
points = points[
    jax.random.choice(jax.random.PRNGKey(0), points.shape[0], (1000,), replace=False), :
]

# vertex_colors_random =  jax.random.uniform(jax.random.PRNGKey(0), (len(merged_vertex_colors[vertex_to_index > 0]), 3))

random_subset = jax.random.choice(
    jax.random.PRNGKey(0),
    jnp.arange(len(merged_faces_no_background)),
    (4000,),
    replace=False,
)
...
for i in range(len(camera_poses_over_time)):
    ...
    rgb_rerender = renderer.render(
        transformed_vertices, merged_faces, merged_vertex_colors
    )
    ...
    rgb_rerender = renderer.render(
        transformed_vertices,
        merged_faces_no_background[random_subset],
        merged_vertex_colors_no_background,
    )
    ...
    pixel_coords = pixel_coords[
        (pixel_coords[:, 0] >= 0)
        & (pixel_coords[:, 0] < image_height)
        & (pixel_coords[:, 1] >= 0)
        & (pixel_coords[:, 1] < image_width)
    ]
    ...
    rgb_rerender = renderer.render(
        transformed_vertices, merged_faces2, merged_vertex_colors2
    )
...
new_camera_pose = (
    camera_pose
    @ Pose.from_translation(jnp.array([0.1, 0.05, 0.3]))
    @ Pose.from_quat(b3d.Rot.from_rotvec(jnp.array([0.0, -0.9, 0.0])).as_quat())
)
rgb_rerender = renderer.render(
    new_camera_pose.inv().apply(merged_vertices), merged_faces, merged_vertex_colors
)
b3d.get_rgb_pil_image(rgb_rerender).save("0.png")
...
for i in range(len(interpolated_camera_poses)):
    rgb_rerender = renderer.render(
        interpolated_camera_poses[i].inv().apply(merged_vertices),
        merged_faces,
        merged_vertex_colors,
    )
...
rendered_pil_images = [
    b3d.get_rgb_pil_image(r) for r, _ in (visualization_images + new_images)
]
observed_pil_images = [
    b3d.get_rgb_pil_image(r) for _, r in (visualization_images + new_images)
]
