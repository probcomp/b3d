...
def get_vertex_depth(v):
    ...
    n_valid = jnp.sum(face_indices != -1)
    return depths[0] #jnp.sum(depths) / n_valid
...
trace, weight = jax.jit(model.importance)(
    key,
    genjax.choice_map({
        "vertices": camera_poses[0].apply(initial_mesh[0]),
...
m.rr_log_trace(trace, renderer, "trace", [0], [0])