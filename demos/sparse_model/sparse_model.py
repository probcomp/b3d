...
object_vertices = object_vertices[
    jax.random.choice(
        ...
        jnp.arange(object_vertices.shape[0]),
        (500,),
        replace=False,
    )
]
...
def model(params, fx, fy, cx, cy):
    xyz_in_camera_frame = jax.vmap(
        lambda i: b3d.Pose(params["position"][i], params["quaternion"][i]).apply(
            params["xyz"]
        )
    )(jnp.arange(params["position"].shape[0]))
...
N = 10
gt_poses = sample_gaussian_vmf_multiple(
    jax.random.split(jax.random.PRNGKey(1000000), N),
    b3d.Pose.from_translation(jnp.array([0.0, 0.0, 3.0])),
    0.001,
    1.0,
)
xyz = object_vertices
gt_pixel_coordinates = model(
    {"xyz": xyz, "position": gt_poses.pos, "quaternion": gt_poses.quat}, fx, fy, cx, cy
)

poses = sample_gaussian_vmf_multiple(
    jax.random.split(jax.random.PRNGKey(11110), N),
    b3d.Pose.from_translation(jnp.array([0.0, 0.0, 3.0])),
    0.001,
    1.0,
)
...
rr.log(
    "gt",
    rr.DepthImage(
        pixel_coordinates_to_image(gt_pixel_coordinates, image_height, image_width)[0]
    ),
    timeless=True,
)
rr.log(
...
for t in pbar:
    ...
    pixel_coords = model(params, fx, fy, cx, cy)
    reconstruction = pixel_coordinates_to_image(
