...
def viz_pixel_coordinates(pixel_coordinates):
    ...
    image_index = torch.arange(len(pixel_coordinates)).repeat_interleave(pixel_coordinates.shape[1])
    keypoint_index = torch.arange(pixel_coordinates.shape[1]).repeat(len(pixel_coordinates)).float()

    valid = (rounded_pixel_coordinates[..., 0] >= 0) & (rounded_pixel_coordinates[..., 0] < image_height) & (rounded_pixel_coordinates[..., 1] >= 0) & (rounded_pixel_coordinates[..., 1] < image_width)
    img[
        image_index[valid],
...
num_views = 10
object_positions = torch.normal(torch.tile(center_position[None,...], (num_views,1)), std=0.1)
object_quaternions = torch.normal(torch.tile(quaternion[None,...], (num_views,1)), std=2.2)
gt_pixel_coordinates = model(object_positions, object_quaternions, object_vertices).detach()
...
rr.log("gt", rr.DepthImage(viz_pixel_coordinates(gt_pixel_coordinates)[0]), timeless=True)
rr.log("overlay", rr.DepthImage(viz_pixel_coordinates(gt_pixel_coordinates)[0]), timeless=True)
