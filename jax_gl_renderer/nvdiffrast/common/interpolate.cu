...
static __forceinline__ __device__ void InterpolateFwdKernelTemplate(const InterpolateKernelParams p)
{
    ...
    float2 r = ((float2*)p.uvs)[pidx];

    int triIdx = (int)(p.triangles[pidx]) - 1;
    ...
    int vi0 = triValid ? p.faces[triIdx * 3 + 0] : 0;
    int vi1 = triValid ? p.faces[triIdx * 3 + 1] : 0;
    int vi2 = triValid ? p.faces[triIdx * 3 + 2] : 0;
    ...
    const float* a1 = p.attributes + vi1 * p.numAttr;
    const float* a2 = p.attributes + vi2 * p.numAttr;
