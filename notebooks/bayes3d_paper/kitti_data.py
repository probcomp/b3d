...
for t, f in tqdm(enumerate(frames)):
    ...
    velo = dataset.get_velo(t)[:, jnp.array([1, 2, 0])] * jnp.array([-1.0, -1.0, 1.0])
    ...
    depth_image = depth_image.at[pixels[valid_pixels, 0], pixels[valid_pixels, 1]].set(
        velo[valid_pixels, 2]
    )
...
masks_jnp = jnp.array([mask["segmentation"] for mask in masks])

kitti_data_path = (
    b3d.get_assets_path()
    / "shared_data_bucket/foundation_pose_tracking_datasets/kitti_initial_data.npz"
)
np.savez(
    kitti_data_path,
...
#         jnp.ones((xyzs.shape[0], 3)) * resolution * 2.0, colors / 255.0
#     ),
