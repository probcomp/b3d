...
def make_visual(scene=None, object=None, debug=False):
    ...
    for object in object_indices:
        ...
        inferred_poses_data = jnp.load(
            f"SCENE_{scene_id}_OBJECT_INDEX_{object}_POSES.npy.npz", allow_pickle=True
        )
        inferred_poses = Pose(
            inferred_poses_data["position"], inferred_poses_data["quaternion"]
        )
        ...
        for t in tqdm(range(len(all_data))):
            ...
            b3d_inferred_rgbd = renderer.render_rgbd_from_mesh(
            ...
            foundation_predicted_pose = Pose.from_matrix(
                jnp.array(np.loadtxt(filename))
            )
            foundation_inferred_rgbd = renderer.render_rgbd_from_mesh(
                mesh.transform(foundation_predicted_pose)
            )
            ...
            video_frames.append(
                b3d.multi_panel(
                    [
                        actual_viz,
                        b3d.overlay_image(actual_viz, b3d_inferred_viz, alpha=alpha),
                        b3d.overlay_image(
                            actual_viz, foundation_inferred_viz, alpha=alpha
                        ),
                    ],
        ...
        b3d.make_video_from_pil_images(
            video_frames, f"video_scene_{scene_id}_object_{object}.mp4"
        )
