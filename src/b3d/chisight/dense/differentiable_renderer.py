...
def dist_params_to_average(weights, attributes, background_attribute):
    extended_attributes = jnp.concatenate(
        [
            jnp.tile(
                background_attribute, (attributes.shape[0], attributes.shape[1], 1, 1)
            ),
            attributes,
        ],
        axis=2,
...
def get_pixel_attribute_dist_parameters(
    ...
    unique_triangle_indices, weights, barycentric_coords = (
        get_weights_and_barycentric_coords(
            ij, vertices, faces, triangle_intersected_padded, hyperparams_and_intrinsics
        )
    )
    unique_triangle_indices_safe = jnp.where(
        unique_triangle_indices < 0, 0, unique_triangle_indices
    )

    # (U - 1, A)
    attributes_at_triangles = jax.vmap(barycentric_interpolation, in_axes=(0, None, 0))(
        faces[unique_triangle_indices_safe[1:]], vertex_attributes, barycentric_coords
    )

    attributes_at_triangles = jnp.where(
        unique_triangle_indices[1:, None] < 0,
        -jnp.ones_like(attributes_at_triangles),
        attributes_at_triangles,
    )
...
def get_weights_and_barycentric_coords(
    ij, vertices, faces, triangle_intersected_padded, hyperparams_and_intrinsics
):
    """
    Returns:
    ...
    unique_triangle_values = (
        jnp.unique(
            triangle_intersected_padded_in_window,
            size=triangle_intersected_padded_in_window.size,
            fill_value=-1,
        )
        - 1
    )
    unique_triangle_values_safe = jnp.where(
        unique_triangle_values < 0, unique_triangle_values[0], unique_triangle_values
    )
    ...
    signed_dist_scores = jax.nn.sigmoid(
        jnp.sign(signed_dist_values) * signed_dist_values**2 / SIGMA
    )
    ...
    zexp = jnp.exp(jnp.clip(z / GAMMA, -20.0, 20.0))
    ...
    unnorm_weights = jnp.concatenate(
        [jnp.array([jnp.exp(jnp.clip(EPSILON / GAMMA, -20, 20))]), unnorm_weights]
    )
    weights = unnorm_weights / jnp.sum(unnorm_weights)

    extended_triangle_indices = jnp.concatenate(
        [jnp.array([-10]), unique_triangle_values]
    )
    barycentric_coords = jnp.where(
        unique_triangle_values[:, None] >= 0,
        barycentric_coords,
        -jnp.ones_like(barycentric_coords),
    )
...
def get_signed_dists_and_barycentric_coords(
    ij, unique_triangle_values, vertices, faces, hyperparams_and_intrinsics
):
    return jax.vmap(
        get_signed_dist_and_barycentric_coords, in_axes=(None, 0, None, None, None)
    )(ij, unique_triangle_values, vertices, faces, hyperparams_and_intrinsics)


def get_signed_dist_and_barycentric_coords(
    ij, triangle_idx, vertices, faces, hyperparams_and_intrinsics
):
    """
    ...
    triangle = vertices[faces[triangle_idx]]  # 3 x 3 (face_idx, vertex_idx)
    point_on_plane = project_pixel_to_plane(ij, triangle, hyperparams_and_intrinsics)
    (in_triangle, (a, b, c, d)) = pt_is_in_plane(triangle, point_on_plane)
...
def pt_is_in_plane(triangle, point_on_plane):
    ...
    is_in_plane = jnp.logical_and(
        jnp.equal(jnp.sign(a), jnp.sign(b)), jnp.equal(jnp.sign(b), jnp.sign(c))
    )

    return (is_in_plane, (a, b, c, d))
...
def project_pixel_to_plane(ij, triangle, hyperparams_and_intrinsics):
    ...
    """
    fx, fy, cx, cy = (
        hyperparams_and_intrinsics.fx,
        hyperparams_and_intrinsics.fy,
        hyperparams_and_intrinsics.cx,
        hyperparams_and_intrinsics.cy,
    )
    y, x = ij
    x_c = (x - cx) / fx
    y_c = (y - cy) / fy
    ...
    return project_ray_to_plane(ray_origin, ray_dir, triangle)


def project_ray_to_plane(ray_origin, ray_dir, triangle):
    ...
    return jnp.where(jnp.abs(denom) < 1e-6, -jnp.ones(3), intersection_point)
...
def dist_to_line_seg(a, b, p):
    ...
    projln = jnp.clip(projln, 0.0, 1.0)
