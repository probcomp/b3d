...
def make_krays_image_observation_model(renderer):
    ...
    class KRaysImageLikelihood(genjax.ExactDensity):
        ...
        def logpdf(self, observed_rgbd, rendered_rgbd, likelihood_args):
            ...
            corrected_depth = (
                rendered_rgbd[..., 3] + (rendered_rgbd[..., 3] == 0.0) * renderer.far
            )
    ...
    def krays_image_observation_model(mesh, likelihood_args):
        noiseless_rendered_rgbd = renderer.render_rgbd(
            mesh.vertices, mesh.faces, mesh.vertex_attributes
        )
        image = (
            krays_image_likelihood(noiseless_rendered_rgbd, likelihood_args) @ "image"
        )
