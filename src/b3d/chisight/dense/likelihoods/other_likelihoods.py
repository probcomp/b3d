...
<<<<<<< HEAD:b3d/chisight/dense/likelihoods/other_likelihoods.py
def normalize(l):
    return jnp.where(
...
>>>>>>> main:src/b3d/chisight/dense/likelihoods/other_likelihoods.py
    )
...
class ImageDistFromPixelDist(genjax.ExactDensity):
    ...
    def logpdf(self, observed_image, height, width, *pixel_dist_args):
        ...
        logpdfs = jax.vmap(
            ...
            observed_image.reshape(-1, observed_image.shape[-1]),
            *self._flattened_args(pixel_dist_args),
        )
        return logpdfs.sum()
...
class UniformRGBDPixelModel(genjax.ExactDensity):
    ...
    def logpdf(self, observed_rgbd, mindepth, maxdepth):
        return b3d.modeling_utils.uniform.logpdf(
            observed_rgbd[:3], jnp.zeros(3), jnp.ones(3)
        ) + b3d.modeling_utils.uniform.logpdf(observed_rgbd[3], mindepth, maxdepth)


uniform_rgbd_pixel_model = UniformRGBDPixelModel()

uniform_rgbd_image_model = ImageDistFromPixelDist(
    uniform_rgbd_pixel_model, [False, False]
)
image_sample = uniform_rgbd_image_model.sample(
...
class RGBDPixelModel(genjax.ExactDensity):
    ...
    def logpdf(self, observed_rgbd, rendered_rgbd, depth_args, color_args):
        rgb_logpdf = self.color_pixel_model.logpdf(
            observed_rgbd[:3], rendered_rgbd[:3], *color_args
        )
        depth_logpdf = self.depth_pixel_model.logpdf(
            observed_rgbd[3], rendered_rgbd[3], *depth_args
        )
...
laplace_rgb_uniform_depth_pixel_model = RGBDPixelModel(
    ArgMap(b3d.modeling_utils.uniform, lambda r, low, high: (low, high)), laplace
)
uniform_rgb_laplace_depth_pixel_model = RGBDPixelModel(
    laplace, ArgMap(b3d.modeling_utils.uniform, lambda r: (jnp.zeros(3), jnp.ones(3)))
)
s1 = laplace_rgbd_pixel_model.sample(
    jax.random.PRNGKey(0), jnp.array([0.5, 0.5, 0.5, 1.5]), (0.1,), (0.9,)
)
...
class VmapMixturePixelModel(genjax.ExactDensity):
    ...
    def sample(self, key, probs, *args):
        ...
        return self.dist.sample(
            key, *[jtu.tree_map(lambda x: x[component], a) for a in args]
        )

    def logpdf(self, observed, probs, *args):
        logprobs = jax.vmap(
            lambda component: self.dist.logpdf(
                observed, *[jtu.tree_map(lambda x: x[component], a) for a in args]
            )
...
uniform_multilaplace_mixture = ArgMap(
    ...
    lambda probs, rgbds, depth_scale, color_scale, mindepth, maxdepth: (
        ...
        [(mindepth, maxdepth), (normalize(probs[1:]), rgbds, depth_scale, color_scale)],
    ),
)

mixture_of_uniform_and_multi_uniformrgb_laplacedepth = ArgMap(
    PythonMixturePixelModel(
        [uniform_rgbd_pixel_model, multi_uniform_rgb_depth_laplace]
    ),
    lambda probs, rgbds, depth_scale, mindepth, maxdepth: (
        jnp.array([probs[0], jnp.sum(probs[1:])]),
        [(mindepth, maxdepth), (normalize(probs[1:]), rgbds, depth_scale)],
    ),
)
...
def get_uniform_multilaplace_image_dist_with_fixed_params(
    ...
    return ArgMap(
        ImageDistFromPixelDist(
            uniform_multilaplace_mixture, [True, True, False, False, False, False]
        ),
...
uniform_multilaplace_mixture_rgb_only = ArgMap(
    PythonMixturePixelModel(
        [b3d.modeling_utils.uniform, multilaplace_pixel_model_rgb_only]
    ),
    lambda probs, rgbs, color_scale: (
        jnp.array([probs[0], jnp.sum(probs[1:])]),
        [(jnp.zeros(3), jnp.ones(3)), (normalize(probs[1:]), rgbs, color_scale)],
    ),
)
...
def get_uniform_multilaplace_rgbonly_image_dist_with_fixed_params(
    ...
    return ArgMap(
        ImageDistFromPixelDist(
            uniform_multilaplace_mixture_rgb_only, [True, True, False]
        ),
