...
def propose_color_and_visibility(trace, key):
    ...
    visibility_values = hyperparams["visibility_values"]

    visibility_sweep = (
        hyperparams["visibility_values"][..., None]  # (num_outlier_grid_points, 1)
    ...
    visibility_transition_scores_per_sweep_point_and_vertex = jnp.vectorize(
        visibility_transition_kernel.logpdf, signature="(),()->()", excluded=(2,)
    )(visibility_sweep, previous_visibility, hyperparams["visibility_values"])
    ...
    color_transition_scores_per_sweep_point_and_vertex = jnp.vectorize(
        color_transition_kernel.logpdf,
        signature="(3),(3)->()",
    ...
    def get_per_vertex_likelihoods_with_new_color_and_color_outlier_probabilities(
        ...
        return info_from_trace(
            trace.update(
                key,
                make_visibiliy_choicemap(visibility) ^ make_colors_choicemap(colors),
            )[0]
        )["scores"]
    ...
    color_shift_scale = hyperparams["color_transition_kernel"].scale
...
def propose_update(trace, key, pose):
    ...
    trace = trace.update(
        ...
        ^ make_visibiliy_choicemap(sampled_visibility),
    )[0]
    total_log_q += log_q
