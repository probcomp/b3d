...
class PoseTests(unittest.TestCase):
    ...
    def test_pose_inv(self):
        ...
        self.assertTrue(
            jnp.allclose(Rot.from_quat(qs).as_matrix(), 
            Rot.from_quat(jnp.array(-1)*qs).as_matrix(), atol=1e-4))

        computed = ps@ps.inv()
        expected = Pose(jnp.zeros((N,3)), jnp.tile(jnp.array([0.,0.,0.,1.]), (N,1)))

        self.assertTrue(jnp.allclose(
            computed.pos, expected.pos, 
            atol=1e-4), computed.pos - expected.pos)

        self.assertTrue(jnp.allclose(
            choose_good_quat(computed.xyzw), expected.xyzw, 
            atol=1e-4), computed.xyzw - expected.xyzw)

    def test_compose(self):
        key = self.key
        keys = keysplit(key, 4)
        a = Pose(
            jax.random.uniform(keys[0], (3,)),
            jax.random.uniform(keys[1], (4,))
        )
        b = Pose(
            jax.random.uniform(keys[2], (3,)),
            jax.random.uniform(keys[3], (4,))
        )   
    ...
    def test_camera_pose_from_position_target(self):
        ...
        camera_vector = Rot.from_quat(pose.xyzw).apply(jnp.array([0., 0., 1.]))

        self.assertTrue( jnp.allclose(camera_vector, (target - pos) / jnp.linalg.norm(target - pos)) )
    ...
    def test_grad(self):
        ...
        for _ in range(10):
            ...
            pose = Pose(
                pose.pos - 0.001 * grad.pos,
                pose.xyzw - 0.001 * grad.xyzw
            )
