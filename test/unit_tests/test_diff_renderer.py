...
def test_diff_renderer(renderer):
    ...
    def loss_func_rgbd(params, gt):
        image = render(params["particle_centers"], params["particle_widths"], params["particle_colors"])
        return jnp.mean(jnp.abs(image[...,:3] - gt[...,:3])) + jnp.mean(jnp.abs(image[...,3] - gt[...,3]))
    ...
    for (title, tx, params) in test_cases:
        ...
        images = [render_jit(params["particle_centers"], params["particle_widths"], params["particle_colors"])]
        for t in pbar:
            params, state, loss = update_params(tx, params, gt_image, state)
            pbar.set_description(f"Loss: {loss}")
            images.append(render_jit(params["particle_centers"], params["particle_widths"], params["particle_colors"]))
