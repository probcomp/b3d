...
class TrianglePosteriorGridApproximationTask(Task):
    ...
    def assert_passing(
        ...
        assert (
            metrics["mass_assigned_outside_expected_region"]
            <= mass_outside_region_threshold
        )
        assert (
            metrics["divergence_from_uniform_in_expected_region"]
            <= divergence_from_uniform_threshold
        )
    ...
    def visualize_solution(self, solution, metrics):
        ...
        self.visualize_grid_approximation(
            jnp.exp(metrics["log_posterior_approximation"]), show_expected_region=False
        )
        self.visualize_grid_approximation(
            jnp.exp(metrics["expected_log_posterior"]), name="expected_grid_posterior"
        )
        self.visualize_log_grid_approximation_2D(
            partition,
            metrics["log_posterior_approximation"],
            name="log_grid_posterior_approximation",
        )
        self.visualize_log_grid_approximation_2D(
            partition,
            metrics["expected_log_posterior"],
            name="expected_log_grid_posterior",
        )
    ...
    def score_oneframe_grid_approximation(self, solution):
        ...
        max_idx_before_region = jnp.argmax(0.0 < partition)
        min_idx_after_region = jnp.argmax(background_z < partition)
        mass_assigned_outside_expected_region = jnp.sum(
            posterior_approximation[:max_idx_before_region]
        ) + jnp.sum(posterior_approximation[min_idx_after_region:])
        posterior_in_expected_region = posterior_approximation[
            max_idx_before_region:min_idx_after_region
        ]
        posterior_in_expected_region = posterior_in_expected_region / jnp.sum(
            posterior_in_expected_region
        )
        uniform_in_expected_region = jnp.ones_like(posterior_in_expected_region) / len(
            posterior_in_expected_region
        )
        divergence_from_uniform = _kl(
            uniform_in_expected_region, posterior_in_expected_region
        )
        expected_posterior = jnp.zeros_like(partition[:-1])
        expected_posterior = expected_posterior.at[
            max_idx_before_region:min_idx_after_region
        ].set(uniform_in_expected_region)
    ...
    def _get_grid_partition(self):
        ...
        def get_int_z(face_idx):
            int_pt = differentiable_renderer.project_ray_to_plane(
                jnp.zeros(3),
                triangle_C[0],
                background_vertices_C[background_faces[face_idx]],
            )
            # print(int_pt.shape)
            # print(background_vertices_C[background_faces[face_idx]].shape)
            is_in_triangle, _ = differentiable_renderer.pt_is_in_plane(
                background_vertices_C[background_faces[face_idx]], int_pt
            )
            # set z to infinity if the intersection point is not in the triangle
            return jnp.where(is_in_triangle, int_pt[2], jnp.inf)
        ...
        jax.experimental.checkify.check(
            not jnp.isinf(background_z), "No intersection found"
        )
    ...
    def default_scene_using_colors(cls, background_color, triangle_color):
        return cls(
            **cls.generate_unichromatic_background_single_frame_taskargs(
    ...
    def generate_unichromatic_background_single_frame_taskargs(
        ...
        return {
            "scene_background": cls.get_monochrome_room_background(background_color),
            "foreground_triangle": {
                "color": triangle_color,
                "vertices": jnp.array(
                    [[5, 3, 7], [6, 1.5, 7], [6, 1.5, 8]], dtype=float
                ),
            },
            "triangle_path": b3d.Pose.identity()[None, ...],
            "camera_path": b3d.pose.camera_from_position_and_target(
                position=jnp.array([5.0, -0.5, 6.0]), target=jnp.array([5, 3, 7.0])
            )[None, ...],
            "taskname": taskname,
        }
    ...
    def visualize_log_grid_approximation_2D(
        self,
        partition,
    ...
    def visualize_grid_approximation(
        self,
        posterior_approximation,
        name="grid_posterior_approximation",
        show_expected_region=True,
    ):
        self.maybe_initialize_rerun()
        partition, background_z = self._get_grid_partition()
        pts_C = partition[:, None] * jnp.array([[0.0, 0.0, 1.0]])
        X_WC = self.camera_path[0]
        ...
        rr.log(
            ...
            rr.LineStrips3D(
                ...
                colors=[
                    p_to_color(a) for a in jnp.clip(posterior_approximation, 0.0, 1.0)
                ],
            ),
        )
        ...
        if show_expected_region:
            ...
            rr.log(
                ...
                rr.LineStrips3D(
                    [
                        [
                            pts_W[max_idx_before_region + 1],
                            pts_W[min_idx_after_region - 1],
                        ]
                    ],
                    colors=[[0.0, 1.0, 1.0]],
                ),
    ...
    def get_camera_frame_scene(self, frame_idx):
        ...
        faces = jnp.concatenate(
            [b_faces, jnp.arange(3).reshape((1, 3)) + b_vertices.shape[0]], axis=0
        )
    ...
    def get_image_at_frame(self, frame_idx):
        ...
        vertices, faces, vertex_colors = (
            b3d.utils.triangle_color_mesh_to_vertex_color_mesh(*scene)
        )
    ...
    def visualize_scene(self):
        ...
        rr.log(
            "/3D/scene/background",
            rr.Mesh3D(vertex_positions=v, triangle_indices=f, vertex_colors=vc),
        )
        rr.log(
            "/3D/scene/foreground",
            rr.Mesh3D(
