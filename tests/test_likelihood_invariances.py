...
def test_resolution_invariance(renderer):
    ...
    for SCALING_FACTOR in [2, 3, 4, 5, 6, 7, 8]:
        ...
        depth_resized = jax.image.resize(
            depth_near,
            (
                depth_near.shape[0] * SCALING_FACTOR,
                depth_near.shape[1] * SCALING_FACTOR,
            ),
            "nearest",
        )
        scaled_up_logpdf = bayes3d.rgbd_sensor_model.logpdf(
            (rgb_resized, depth_resized),
            rgb_resized,
            depth_resized,
            model_args,
            fx * SCALING_FACTOR,
...
def test_distance_to_camera_invarance(renderer):
    ...
    object_library.add_object(
        occluder.vertices, occluder.faces, attributes=occluder_colors
    )
    ...
    near_score = bayes3d.rgbd_sensor_model.logpdf(
        (rgb_near, depth_near), rgb_near, depth_near, model_args, fx, fy, 0.0
    )

    far_score = bayes3d.rgbd_sensor_model.logpdf(
