...
class MeshTests(unittest.TestCase):
    def test_merge_meshes(self):
        ...
        assert (
            combined_mesh.vertices.shape[0]
            == mesh1.vertices.shape[0] + mesh2.vertices.shape[0]
        )
        assert (
            combined_mesh.vertex_attributes.shape[0]
            == mesh1.vertex_attributes.shape[0] + mesh2.vertex_attributes.shape[0]
        )
        assert (
            combined_mesh.faces.shape[0] == mesh1.faces.shape[0] + mesh2.faces.shape[0]
        )
        assert (
            combined_mesh.vertices == jnp.concatenate([mesh1.vertices, mesh2.vertices])
        ).all()
        assert (
            combined_mesh.vertex_attributes
            == jnp.concatenate([mesh1.vertex_attributes, mesh2.vertex_attributes])
        ).all()
    ...
    def test_vmapping_mesh(self):
        ...
        renderer.render_rgbd_from_mesh(
            merged_mesh.transform(Pose.from_translation(jnp.array([0, 0, 2.1])))
        )
